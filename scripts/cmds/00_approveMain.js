// This is the "Ultra Pro Max" version of the 'main' command, renamed to 'maincmd'.
...
// --- CONSTANTS ---
const CACHE_DIR = path.join(__dirname, 'cache'); // Using the standard cache directory
const APPROVED_PATH = path.join(CACHE_DIR, 'maincmd_approved.json');
const PENDING_PATH = path.join(CACHE_DIR, 'maincmd_pending.json');

// --- LOCKING MECHANISM (Essential for safe file operations) ---
class Lock {
...
module.exports = {
  config: {
    ...
    author: 'Asif',
    cooldowns: 5,
    role: 0, // Role 0 to allow 'request' subcommand
    category: 'system',
    shortDescription: { en: 'Manage access to main commands.' },
    longDescription: { en: 'A system for users to request, and admins to approve/deny, access to protected commands.' },
    guide: { en: 
        "Admin:\n" +
        "• {p}{n} approve <id> [msg] - Approve a thread.\n" +
        "• {p}{n} deny <id> [reason] - Deny a pending request.\n" +
        "• {p}{n} remove <id> [reason] - Remove an approved thread.\n" +
        "• {p}{n} list [approved|pending] - List threads.\n\n" +
        "User:\n" +
        "• {p}{n} request - Request access for your group."
    }
  },
  ...
  onStart: async function({ api, event, args, GoatBot }) {
    ...
    if (subCommand === 'request') {
        ...
        try {
            ...
            if (approved.includes(threadID)) return api.sendMessage("✅ This thread already has access to main commands.", threadID, messageID);
            if (pending.includes(threadID)) return api.sendMessage("⏳ Your request is already pending review.", threadID, messageID);

            pending.push(threadID);
            await fs.writeJson(PENDING_PATH, pending, { spaces: 2 });

            api.sendMessage("✅ Your request has been sent to the bot admins for review.", threadID, messageID);
            
            const threadInfo = await api.getThreadInfo(threadID);
            const userName = (await api.getUserInfo(senderID))[senderID].name;
            const adminMessage = 
                `🔔 New Main Command Access Request 🔔\n\n` +
                `Group: ${threadInfo.threadName}\n` +
                `ID: ${threadID}\n` +
                `From: ${userName}\n\n` +
                `To approve, use:\nmaincmd approve ${threadID}`;
    ...
    if (!GoatBot.config.admins.includes(senderID)) {
      return api.sendMessage("You don't have permission for this action.", threadID, messageID);
    }
    ...
    try {
      ...
      switch (subCommand) {
        ...
        case 'deny': {
          ...
          await fs.writeJson(PENDING_PATH, pending.filter(id => id !== targetID), { spaces: 2 });
          api.sendMessage(`🚫 Denied request for thread ${targetID}.`, threadID, messageID);
          break;
        }
        case 'remove': {
          ...
          api.sendMessage(`🗑️ Removed access for thread ${targetID}.`, threadID, messageID);
          break;
        }
        case 'list': {
          const listType = (args[1] || 'approved').toLowerCase();
          const list = listType === 'pending' ? pending : approved;
          const title = listType === 'pending' ? '⏳ Pending Threads' : '✅ Approved Threads';
        ...
        default: {
          api.sendMessage(this.config.guide.en, threadID, messageID);
          break;
        }
      }
    } finally {
