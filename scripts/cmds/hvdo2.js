...
module.exports = {
  config: {
    ...
    countDown: 60,
    role: 2,
    shortDescription: "random hentai video",
    longDescription: "Random hentai video dibe, repeat hobe na jotokkhon na sob dekho.",
    category: "18+",
    guide: "{p}{n}hvdo",
  },
  ...
  onStart: async function ({ api, event, message }) {
    // Banglish: Loading message pathai
    let loadingMessage;
    try {
      const senderID = event.senderID;

      loadingMessage = await message.reply({
        body: "Loading random hentai... Please wait! Up to 5min 🤡",
      });

      // Banglish: Video link gula ekhane full list
      const link = [
      ...
      // Banglish: Notun video gula ber kori
      let availableVideos = link.filter(video => !this.sentVideos.includes(video));

      // Banglish: Jodi sob already pathano, abar shuru kori
      if (availableVideos.length === 0) {
        this.sentVideos = [];
        availableVideos = [...link];
      }

      // Banglish: Random video select kori
      const randomIndex = Math.floor(Math.random() * availableVideos.length);
      const randomVideo = availableVideos[randomIndex];
      this.sentVideos.push(randomVideo);

      // Banglish: Video thik ase kina check kori
      try {
        const headRes = await axios.head(randomVideo);
        if (headRes.status !== 200) throw new Error("Video not found");
      } catch (err) {
        return message.reply("❌ Video link kaj korche na, abar try koro.");
      }

      // Banglish: Video pathai
      await message.reply({
        body: 'Full video dekho, skip korba na! 🥵',
        attachment: await global.utils.getStreamFromURL(randomVideo),
      });

      // Banglish: Loading message delete kori
      ...
      // Banglish: Error hoile message pathai
      if (loadingMessage && loadingMessage.messageID) {
        api.unsendMessage(loadingMessage.messageID);
      }
      return message.reply("❌ Error hoise, abar try koro. (API ba link problem hote pare)");
    }
  },
};
